...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const
{
    ...
    std::string shaderErrorLog = checkForShaderCompilationErrors(shader);

    if (!shaderErrorLog.empty())
    {
        std::cerr << "ERROR IN SHADER COMPILATION" << shaderErrorLog;
        return false;
    }

    glAttachShader(program, shader);

    glDeleteShader(shader); // Delete shader (the shader is already attached to the program so its object is no longer needed)
...
bool our::ShaderProgram::link() const
{
    ...
    std::string LinkingErrorLog = checkForLinkingErrors(program);

    if (!LinkingErrorLog.empty())
    {
        std::cerr << "LINKING ERROR: " << LinkingErrorLog;
        return false;
    }
    return true;
}
