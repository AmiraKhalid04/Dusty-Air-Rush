    ...
    void AssetLoader<Mesh>::deserialize(const nlohmann::json &data)
    {
        if (data.is_object())
        {
            for (auto &[name, desc] : data.items())
            {
                ...
                // ---- String: treat as OBJ file path (original behavior) ----
                if (desc.is_string())
                {
                    std::string path = desc.get<std::string>();
                    assets[name] = mesh_utils::loadOBJ(path);

                    // ---- Object: procedural mesh generation ----
                }
                else if (desc.is_object())
                {
                    std::string procedural = desc.value("procedural", "");

                    if (procedural == "heightmap")
                    {
                        std::string hmPath = desc.value("heightmap", "");
                        int width = desc.value("width", 480);
                        int depth = desc.value("depth", 480);
                        float hScale = desc.value("heightScale", 20.0f);
                        float wScale = desc.value("worldScale", 100.0f);
                        float uvScale = desc.value("uvScale", 16.0f);
                        assets[name] = mesh_utils::heightmap(hmPath, width, depth, hScale, wScale, uvScale);
                    }
                    else if (procedural == "cloudPlane")
                    {
                        int segments = desc.value("segments", 4);
                        assets[name] = mesh_utils::cloudPlane(segments);
                    }
                }
            }
        }
