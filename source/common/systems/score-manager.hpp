...
namespace our
{
    ...
    public:
        ...
        // Compute the final score from in-game stats.
        // Formula: (rings × 20) + (coins × 10) + floor(healthPercent)
        static int computeFinalScore(int rings, int coins, float currentHealth, float maxHealth)
        {
            float healthPercent = (maxHealth > 0.0f) ? (currentHealth / maxHealth) * 100.0f : 0.0f;
            return (rings * 20) + (coins * 10) + static_cast<int>(std::floor(healthPercent));
        }
